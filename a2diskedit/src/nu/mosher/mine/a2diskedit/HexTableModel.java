...
//      return byteToHex(hi) + byteToHex(lo);
		int b0 = (val & 0x000000ff) >> 0*8;
		int b1 = (val & 0x0000ff00) >> 1*8;
		int b2 = (val & 0x00ff0000) >> 2*8;
		int b3 = (val & 0xff000000) >> 3*8;

		StringBuffer s = new StringBuffer(8);

		s.append(byteToHex(b3));
		s.append(byteToHex(b2));
		s.append(byteToHex(b1));
		s.append(byteToHex(b0));
   ...
   public Object getValueAt(int row, int col)
   {
      ...
      if (row == (data.getRowCount() - 1) && col >= data.getLastRowSize())
      {
         return "";
      }

 ...
 */
/**
 * Nibble-encodes/decodes a byte using Apple ][ "5 and 3" encoding. This
 * encoding is used on the "13-sector" disks, for Apple ][ DOS 3.1 and 3.2 (not
 * 3.3).
 * 
...
public class Nibblizer5and3 {
  ...
  /**
   * Encodes the given sector (256 bytes) using the <i>alternate</i> "5 and 3"
   * encoding scheme.
   * 
   * @param data
   *          sector to encode
   * @return encoded sector (411 bytes)
   */
  ...
  /**
   * Decodes the given sector (256 logical bytes, 411 actual bytes) using the
   * <i>alternate</i> "5 and 3" encoding scheme. Note that the <i>alternate</i>
   * scheme is used on Track Zero Sector Zero (only), of the disk.
   * 
   * @param enc
   *          encoded sector
  ...
  /**
   * Decodes the given sector (256 logical bytes, 411 actual bytes) using the
   * normal "5 and 3" encoding scheme. Based on code by Andy McFadden, from
   * CiderPress.
   * 
   * @param enc
   *          encoded sector
  ...
  public static int[] decode(final int[] enc) throws CorruptDataException {
    ...
    chksum ^= val;
    if (chksum != 0) {
      // TODO from a data-recovery point of view, isn't there something else we
      // could do?

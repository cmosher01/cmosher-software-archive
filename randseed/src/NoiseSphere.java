...
public class NoiseSphere extends Applet implements Runnable
{
	...
	void plot(Cartesian c)
	{
		try
		{
			imageData[(MidA + round(Scale * c.y)) * myWidth + MidA - round(Scale * c.z)] = 0xffd0d0d0;
			imageData[(MidA + round(Scale * c.x)) * myWidth + MidB - round(Scale * c.y)] = 0xffd0d0d0;
			imageData[(MidB + round(Scale * c.z)) * myWidth + MidA - round(Scale * c.x)] = 0xffd0d0d0;
		}
	...
	public void update(Graphics g)
	{
		...
		offscreen.drawString("Premere 'c' per ricominciare.",x,line(fm,y,6));

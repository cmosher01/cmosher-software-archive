 ...
 *
 */
package com.surveysampling.mosher.tz;
...
public class ZoneInfo extends TimeZone {
  ...
  public ZoneInfo(String tzname) throws IOException {
    this(new File("/etc/zoneinfo",tzname));
  }
  ...
  public int getOffset(int era,int year,int month,int day,int dow,int millis) {
    ...
    try {
      secs = mktime(then,true);
    }
    catch (IllegalArgumentException x) {
      // outside the range of mktime
      return getRawOffset();
    }
    int offset = getTZ(secs).offset;
...
/*
 *      Stdio.sprintf(buf,"%2d/%02d/%04d %2d:%02d:%02d %s", new Object[] {
	new Integer(tm_mon + 1), new Integer(tm_mday),
	new Integer(tm_year + 1900), new Integer(tm_hour),
	new Integer(tm_min), new Integer(tm_sec),
...
*/
     ...
     */
    public void setClock(int clock,int offset) {
      int days = clock / SECSPERDAY;
      int tm_secs = clock % SECSPERDAY;
      tm_secs += offset;
      while (tm_secs < 0) {
	tm_secs += SECSPERDAY;
	--days;
   ...
   */
  public int mktime(tm yourtm) {
    return mktime(yourtm,false);
  }

  private int mktime(tm yourtm,boolean raw) {
    int t = 0;
    int bits = 31;
   ...
   */
  private int timesub(int clock, TZType tz, tm t) {
    boolean		hit = false;

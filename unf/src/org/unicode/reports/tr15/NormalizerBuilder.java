...
if (cc != 0)
	System.err.println("non-zero canonical class (good) for: "+value);
            if (cc != (cc & 0xFF))
            ...
            if (start != end)
            {
                ...
                if (GENERATING)
                {
                    if (value < 0xFF)
                    {
                        System.out.println("\"\\u" + hex((char)value) + "\", " + "\"\\u" + hex(decomp, "\\u") + "\", " + (compat ? "\"K\"," : "\"\",") + "// " + name);
                    }
                }
                ...
                if (!compat && !isExcluded.get((char)value))
                {
                    char first = '\u0000';
        ...
        if (DEBUG)
            System.out.println("Adding Hangul");

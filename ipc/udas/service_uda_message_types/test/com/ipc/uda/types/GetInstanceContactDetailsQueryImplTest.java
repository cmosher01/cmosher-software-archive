 ...
 * 
 */
public class GetInstanceContactDetailsQueryImplTest extends TestCase
{
    ...
    public void testExecute() throws ExecutionException
    {
        ...
        final GetInstanceContactDetailsQueryImpl instanceContactDetailsQry = new GetInstanceContactDetailsQueryImpl();
        instanceContactDetailsQry.setUserContext(user);
        final UID uid = new UID("" + 1);
        instanceContactDetailsQry.setContactId(uid);
        qry.setGetInstanceContactDetails(instanceContactDetailsQry);
        req.setQuery(qry);
        final Executable exe = req.getExecutable();
        Assert.assertTrue(exe instanceof GetInstanceContactDetailsQueryImpl);

        final Optional<ExecutionResult> exeRes = exe.execute();
        final QueryResult exeResult = (QueryResult) exeRes.get();
        final InstanceContactDetailsResult perCtDetailRes = exeResult.getInstanceContactDetails();

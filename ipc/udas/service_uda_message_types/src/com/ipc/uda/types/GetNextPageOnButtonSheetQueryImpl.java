 ...
 * 
 */
public class GetNextPageOnButtonSheetQueryImpl extends GetNextPageOnButtonSheetQuery implements
...
// REVIEW: 10503 This impl is no longer needed and should be removed
        ...
        // wrap the pages
        if (this.getCurrentPageNumber() == ButtonSheet.MAX_NUM_OF_PAGES_ON_BUTTON_SHEET)
        {
            pageMap = this.ctx.getCallContext().getButtonSheet().getButtonsOfPage(
                    GetNextPageOnButtonSheetQueryImpl.FIRST_PAGE_NUM);
        }
        else
        // get the next page of the button sheet
        {
            pageMap = this.ctx.getCallContext().getButtonSheet().getButtonsOfPage(

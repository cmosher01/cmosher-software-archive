...
#
# Usage: /etc/init.d/bulkemailerdaemon_webuser.ksh {start|stop}
#
case "$1" in
'start')
        echo " Starting Bulk Emailer"
        su - webuser -c "at -q b -k -f /usr/ssi/deploy/app/bulkemailer/\${SWLEVEL}/scripts/bem.ksh now"
;;
'stop')
        echo " Killing Bulk Emailer"
        echo "POST /shutdown" >/dev/tcp/127.0.0.1/60000
;;
*)
        echo "Usage: /etc/init.d/bulkemailerdaemon_webuser.ksh {start|stop}"
        exit 1
;;
esac

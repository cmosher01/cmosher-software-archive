...
//	set coMy = Server.CreateObject("Groinproc.GroGetData")
	set coMy = Server.CreateObject("Gro.GroGetData")
...
<% while (rs.EOF = false) %>
	<TR>
		<TD><B><A HREF="default.asp?docid=<% = rs.Fields("title") %>&iLevel=0&iBase=0"><% = rs.Fields("title") %></A></B></TD>
		<TD><% = DataField(rs.Fields("notes")) %></TD>
	</TR>
	<% rs.MoveNext %>
<% wend %>
</TABLE>
...
<% if iIndi >= 0 then %>
(<A HREF="/geneal/geneal.html">home</A>)
...
<% if not (rsEvts.EOF = true) then %>
	...
	<% while not (rsEvts.EOF = true) %>
		<TR>
			<TD nowrap="nowrap" valign="top"><B><% = DataField(rsEvts.Fields("name")) %></B></TD>
			<TD nowrap="nowrap" valign="top"><% = DataField(rsEvts.Fields("date")) %></TD>
			<TD nowrap="nowrap" valign="top"><% = DataField(rsEvts.Fields("place")) %></TD>
			<TD valign="top"><% = DataField(rsEvts.Fields("notes")) %></TD>
			<TD valign="top"><FONT size="1"><CITE>
...
<% if not (rsAttrs.EOF = true) then %>
	...
	<% while not (rsAttrs.EOF = true) %>
		<TR>
			<TD nowrap="nowrap" valign="top"><B><% = DataField(rsAttrs.Fields("name")) %></B></TD>
			<TD nowrap="nowrap" valign="top"><% = DataField(rsAttrs.Fields("value")) %></TD>
			<TD nowrap="nowrap" valign="top"><% = DataField(rsAttrs.Fields("date")) %></TD>
			<TD nowrap="nowrap" valign="top"><% = DataField(rsAttrs.Fields("place")) %></TD>
			<TD valign="top"><% = DataField(rsAttrs.Fields("notes")) %></TD>
			<TD valign="top"><FONT size="1"><CITE>
		...
		<% if not (rsPEvts.EOF = true) then %>
			<TABLE class="datasheet" cellspacing="1">
				...
				<% while not (rsPEvts.EOF = true) %>
					<TR>
						<TD nowrap="nowrap" valign="top"><B><% = DataField(rsPEvts.Fields("name")) %></B></TD>
						<TD nowrap="nowrap" valign="top"><% = DataField(rsPEvts.Fields("date")) %></TD>
						<TD nowrap="nowrap" valign="top"><% = DataField(rsPEvts.Fields("place")) %></TD>
						<TD valign="top"><% = DataField(rsPEvts.Fields("notes")) %></TD>
						<TD valign="top"><FONT size="1"><CITE>
			...
			<TABLE class="datasheet" cellspacing="1">
				<% while not (rsChildren.EOF = true) %>
					<TR>
						...
						<TD><B>birth</B></TD>
						<TD>
							<%= rsChildren.Fields("birthdate") %>
						</TD>
						<TD>
							<%= rsChildren.Fields("birthplace") %>
						</TD>
					</TR>
					<TR>
						<TD>&nbsp;</TD>
						<TD><B>death</B></TD>
						<TD>
							<%= rsChildren.Fields("deathdate") %>
						</TD>
						<TD>
							<%= rsChildren.Fields("deathplace") %>
						</TD>
					</TR>
					<% rsChildren.MoveNext %>
				<% wend %>
...
<P>
<TABLE>
	<TR><TD class="source" align="right" valign="bottom"><SPAN STYLE="color: gray; font-size: 90%">author:</SPAN></TD><TD class="source"><B><% = sAuthor %>.</TD></TR>
	<TR><TD class="source" align="right" valign="bottom"><SPAN STYLE="color: gray; font-size: 90%">title:</SPAN></TD><TD class="source"><B><U><% = rs.Fields("title") %></U>.</B></TD></TR>
	<TR><TD class="source" align="right" valign="bottom"><SPAN STYLE="color: gray; font-size: 90%">publ.:</SPAN></TD><TD class="source"><SPAN STYLE="font-size: 90%">(<% = sPubl %>).</SPAN></TD></TR>
</TABLE>
...
<% if rs.Fields("text").ActualSize > 0 then %>
		<P><SPAN STYLE="color: gray; font-size: 75%">actual transcription and/or representation of source [with notes in brackets]:</SPAN>
		<HR>
		<% = rs.Fields("text") %>
		<HR>
<% end if %>

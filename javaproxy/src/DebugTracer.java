/**
 * @(#)DebugTracer.java April 3, 1998 I MAKE NO WARRANTIES ABOUT THE SUITABILITY
...
/**
 * DebugTracer is used for tracking the calling functions in the stack. It could
 * be used to print out function names and line numbers for debugging.
 */
public class DebugTracer extends Throwable
{
    ...
    public String getSource(int depth)
    {
        ...
        Vector vt = getVectorStack(depth + 2); // need to add this method's
                                               // level
    ...
    public Vector getVectorStack(int depth)
    {
        ...
        while (st.hasMoreTokens())
        {
            ...
            temp = st.nextToken();
            if (++count < 2)
                continue; // don't count the first 2 level
            l = temp.indexOf("at "); // search for "at "
            if (l >= 0)
                temp = temp.substring(l + 3); // skip over "at "
            vt.addElement(temp);
            if (count >= depth + 1)
                break; // check depth; first 2 level doesn't count
        }
        return vt;
    }

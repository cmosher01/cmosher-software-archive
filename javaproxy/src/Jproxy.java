 ...
 */
class Jproxy extends Frame implements ServerInterface, ActionListener, WindowListener, MouseListener
{
    ...
    Jproxy(int width, int height) {
        ...
        try {   // run media tracker to wait for images
            tracker.waitForID(0);
            tracker.waitForID(1);
            tracker.waitForID(2);
            tracker.waitForID(3);
    ...
    private boolean initialization() {
        ...
        host_port   = Integer.parseInt((String) httpconfig.getProperty("network.port"));

        String cacheMaxString = (String) httpconfig.getProperty("network.cache.max");
        String cacheExpString = (String) httpconfig.getProperty("network.cache.expire");
        if(cacheMaxString == null || cacheMaxString.equals("")) cacheMaxString = "0";
        if(cacheExpString == null || cacheExpString.equals("")) cacheExpString = "0";
        try {
            ...
            nfe.printStackTrace();
            System.out.println("Error: Bad Number Format for cache!. Cache Dieabled.");
            cache_max    = 0;
            cache_expire = 0;
        }
        if(cache_max > 800) cache_max = 800;
        if(cache_max < 0) cache_max = 0;
        if(cache_expire > 720) cache_expire = 720;
        if(cache_expire < 0) cache_expire = 0;
        System.out.println("cache size: " + cache_max + " ,, " + "expires : " + cache_expire);
        ...
        log_level   = Integer.parseInt((String) httpconfig.getProperty("log.loglevel"));
    ...
    public void showServerResponse(String msg) {
        if(serverrespwin != null) serverrespwin.addMessage(msg);
    }
    ...
    public void showClientRequest(String msg) {
        if(clientreqwin != null) clientreqwin.addMessage(msg);
    }
    ...
    public void paint (Graphics g) {
        ...
        if(offscrimg == null) offscrimg = createImage(size().width, size().height);
        Graphics og = offscrimg.getGraphics();

        if( bkgroundimg != null ) {
            while(img_width <= 0 || img_height<= 0) {
            ...
            for(int i = 0; i<= client_area_width/img_width + 1; i++ )
             for(int j = 0; j <= client_area_height/img_height + 1; j++)
                if( i + j > 0 ) {
                    og.copyArea(0, 0, img_width, img_height, i*img_width, j*img_height);
                }
...
*/
    ...
    public boolean startProxyServer(boolean doconfirm) {
            ...
            logAccess(_LOG_LEVEL_MAXIMAL, tracer.getSource(), "starting server on " + host_addr + " port: " + host_port );
            if(proxyserver != null && proxyserver.isServerRunning()) return false;

            proxyserver = new ProxyServer(this, host_addr, host_port);
            if(!proxyserver.startServer()) {
                logAccess(_LOG_LEVEL_MAXIMAL, tracer.getSource(), "Error: failed to start server.");
                proxyserver = null;
                System.gc();
            }
            else {
                ...
                logAccess(_LOG_LEVEL_MAXIMAL, tracer.getSource(), "Server started successfully.");
            }
            return true;
    }
    ...
    public boolean showResp() {
        if(serverrespwin != null) return false; // window exists already
    ...
    public boolean showReq() {
        if(clientreqwin != null) return false; // window exists already
    ...
    public boolean showAccessLog() {
        if(access_log_win != null) return false; // window exists already
    ...
    public boolean showErrorLog() {
        if(error_log_win != null) return false; // window exists already
    ...
    public synchronized void logAccess(int level, String source, String message) {
        if(log_level >= level) access_log.addMessage(source, message);
    }
    ...
    public void actionPerformed(ActionEvent actevt) {
        ...
        if( evtsrc instanceof ImageButton  ) {
            if( evtsrc == ibstart ) startProxyServer(false);
            else if( evtsrc == ibstop ) stopProxyServer(false);
            else if( evtsrc == ibexit ) { System.exit(0); }
            else if( evtsrc == ibviewresp ) showResp();
            else if( evtsrc == ibviewreq ) showReq();
            return;
        }

        // menu item events
        if( evtsrc instanceof MenuItem ) {
            if( actcmd.equals( _START_SERVER ) ) startProxyServer(false);
            else if ( actcmd.equals( _SHUTDOWN_SERVER ) ) stopProxyServer(false);
            else if ( actcmd.equals( _EXIT ) ) System.exit(0);
            else if ( actcmd.equals( _SERVER_RESPONSES ) ) showResp();
            else if ( actcmd.equals( _CLIENT_REQUESTS ) ) showReq();
            else if ( actcmd.equals( _VACCESS_LOG ) ) showAccessLog();
            else if ( actcmd.equals( _VERROR_LOG ) )  showErrorLog();
            else if ( actcmd.equals( _SETUP_PROXY_SERVER ) ) handleHTTPSetup();
            else if ( actcmd.equals( _HELP ) ) showHelp();
            else if ( actcmd.equals( _ABOUT ) ) showAbout();
        }
    }
    ...
    public void processWindowEvent(WindowEvent winevt) {
        if( winevt.getID() == Event.WINDOW_DESTROY ) System.exit(0);
    }
    ...
    public void mouseEntered(MouseEvent e) {
        ...
        if( comp == ibstart ) tbcpt.setCaption("Start Server");
        else if( comp == ibstop ) tbcpt.setCaption("Stop Server");
        else if( comp == ibexit ) tbcpt.setCaption("Exit");
        else if( comp == ibviewresp) tbcpt.setCaption("View Responses");
        else if( comp == ibviewreq) tbcpt.setCaption("View Requests");

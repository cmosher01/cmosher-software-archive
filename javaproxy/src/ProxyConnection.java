/**
 * @(#)ProxyConnection.java	April 3, 1998
 *
...
public class ProxyConnection extends Thread {
    ...
    ProxyConnection(Object c, Object p,  Socket s ) {
        ...
        if(showcontent_str != null) show_content = showcontent_str.equals("true") ? true : false;

        if(proxy_addr != null && !proxy_addr.equals("") && proxy_port >= 0) using_proxy = true;
    ...
    public void run() {
        ...
        // System.out.println("\nClient Request: \n" + sb.toString()); // debug
        ((ServerInterface) console).showClientRequest(sb.toString()); // show message
        ...
        content_len = c_header.getContentLength(); // get content length from client request

        EOH = false; // flag End Of Header
        try {

            if(using_proxy) { // connect to another proxy server if using proxy
                ...
                for (Enumeration enu = ht.keys(); enu.hasMoreElements() ;) {
                    ...
                    if(key.equalsIgnoreCase("Proxy-Connection")) continue; // bypass the proxy connection row
            ...
            if(content_len > 0) {
                ...
                while(content_len > total_len) {
                    ...
                    if(using_cache || show_content) { // save content if using cache
                ...
                if(using_cache || show_content) resp_content = resp_sb.toString().getBytes("8859_2");
            }

...
//-beta 2 released-
//version .12 - fixed EOF problem (-1 from stream) in readServerMessageType
...
class rfbProto {
  ...
  static final int FramebufferUpdate = 0, SetColourMapEntries = 1, Bell = 2,
      ServerCutText = 3, ResizeFrameBuffer = 4, PalmVNCReSizeFrameBuffer = 0xF;

  static final int SetPixelFormat = 0, FixColourMapEntries = 1,
      SetEncodings = 2, FramebufferUpdateRequest = 3, KeyEvent = 4,
      PointerEvent = 5, ClientCutText = 6;

  static final int EncodingRaw = 0, EncodingCopyRect = 1, EncodingRRE = 2,
      EncodingCoRRE = 4, EncodingHextile = 5, EncodingZlib = 6, EncodingTight = 7,
      EncodingZlibHex = 8, EncodingZRLE = 16, EncodingCache = 0xFFFF0000,
	  EncodingCacheEnable = 0xFFFF0001, EncodingXOR_Zlib = 0xFFFF0002,
	  EncodingXORMonoColor_Zlib = 0xFFFF0003, EncodingXORMultiColor_Zlib = 0xFFFF0004,
	  EncodingSolidColor = 0xFFFF0005, EncodingXOREnable = 0xFFFF0006,
	  EncodingCacheZip = 0xFFFF0007, EncodingSolMonoZip = 0xFFFF0008,
  ...
  public static final int KEY_SHIFT              =  0xffe1;
  public static final int KEY_CONTROL            =  0xffe3;
  public static final int KEY_META               =  0xffe7;
  public static final int KEY_ALT                =  0xffe9;
  ...
  rfbProto(String h, int p) throws IOException {
    ...
    //is = new DataInputStream(new RingBufferInputStream(sock.getInputStream()));
  ...
  void readFramebufferUpdateRectHdr() throws IOException {
    ...
    //SolMonoZip needs this check disabled.
  ...
  void writePointerClick(int x, int y) throws IOException {
  		writePointerClick(x,y,1);
  }

  void writePointerClick(int x, int y, int btn) throws IOException {
  	eventBufLen = 0;
  	if (x < 0) x = 0;
    if (y < 0) y = 0;
  ...
  void writeKeyEvent(Event evt) throws IOException {
	//we only want key events in here
	if (!evt.isKeyEvent()) return;

    int key = evt.what;
    boolean down = false;

    if ((evt.type == Event.EVENT_KEY_DOWN))
      down = true;

/*    if ((evt.type == Event.EVENT_KEY_UP)) {
...
*/
	...
	//for now, only accept real keypresses
	int keycode = 0;
  ...
  void sendKeyPressEvent(int keysym, int modifiers) throws IOException{
    eventBufLen = 0;
  ...
  void sendModifiers(int modifiers) throws IOException{
    eventBufLen = 0;
    writeModifierKeyEvents(modifiers);
  	os.write(eventBuf, 0, eventBufLen);
  }

  void sendKeyPressEvent(int keysym) throws IOException{
    eventBufLen = 0;
  	writeKeyEvent(keysym, true);
  	writeKeyEvent(keysym, false);
	os.write(eventBuf, 0, eventBufLen);
  ...
  void writeModifierKeyEvents(int newModifiers)
       throws IOException
  {
   if ((newModifiers & Event.EVENT_MODIFIER_MENU_BUTTON) != (oldModifiers & Event.EVENT_MODIFIER_MENU_BUTTON))
      writeKeyEvent(0xffe3, (newModifiers & Event.EVENT_MODIFIER_MENU_BUTTON) != 0);

    if ((newModifiers & Event.EVENT_MODIFIER_CAPS) != (oldModifiers & Event.EVENT_MODIFIER_CAPS))
      writeKeyEvent(0xffe1, (newModifiers & Event.EVENT_MODIFIER_CAPS) != 0);

    //if ((newModifiers & Event.META_MASK) != (oldModifiers & Event.META_MASK))
    //  writeKeyEvent(0xffe7, (newModifiers & Event.META_MASK) != 0);

    if ((newModifiers & Event.EVENT_MODIFIER_ALT) != (oldModifiers & Event.EVENT_MODIFIER_ALT))
      writeKeyEvent(0xffe9, (newModifiers & Event.EVENT_MODIFIER_ALT) != 0);

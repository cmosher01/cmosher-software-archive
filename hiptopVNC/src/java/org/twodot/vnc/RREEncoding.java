...
public class RREEncoding extends Encoding {

	private long stat_bytes = 0;
	private long stat_rects = 0;

	RREEncoding(rfbProto r, Bitmap b) {
		super(r,b);
	}
	...
	public int  getEncodingID() { return rfbProto.EncodingRRE; }

	public String  getName() { return "RRE"; }
	...
	public void doEncoding(int x, int y, int w, int h) throws IOException {
		...
		for (int j = 0; j < nSubrects; j++) {
			pixel = rfb.is.read();
			sx = rfb.is.readUnsignedShort() + rfb.updateRectX;
			sy = rfb.is.readUnsignedShort() + rfb.updateRectY;
